        ...
        with open(ALI_OWNERS_FILE, 'r') as file:
            ...
            for line in lines:
                try:
                    ...
                    logging.error(f"خطأ في تنسيق السطر: {line}")  # تسجيل السطر الغير صحيح
            return owners
...
def update_owners(a):
    try:
        ...
        Ali = a.from_user
        logging.info(f"Received activation command from chat: {chat_id}")  # تسجيل الرسالة
        ...
        logging.error(f"حدث خطأ أثناء تحديث المالكين: {e}")