...
def start_adding_response(a):
    ...
    user_states[a.chat.id] = {"state": "awaiting_trigger", "user_id": a.from_user.id}

# استقبال كلمة الرد
@bot.message_handler(func=lambda message: user_states.get(message.chat.id) and user_states[message.chat.id]["state"] == "awaiting_trigger" and user_states[message.chat.id]["user_id"] == message.from_user.id)
def get_trigger(a):
    user_states[a.chat.id]["state"] = "awaiting_reply"
    responses[a.chat.id] = {"trigger": a.text.strip(), "user_id": a.from_user.id}
    bot.reply_to(a, "الآن قم بإرسال جواب الرد.")

# استقبال جواب الرد
@bot.message_handler(func=lambda a: user_states.get(a.chat.id) and user_states[a.chat.id]["state"] == "awaiting_reply" and user_states[a.chat.id]["user_id"] == a.from_user.id)
def get_reply(a):
...
def start_deleting_response(a):
    ...
    user_states[a.chat.id] = {"state": "awaiting_deletion", "user_id": a.from_user.id}
...
@bot.message_handler(func=lambda message: user_states.get(message.chat.id) and user_states[message.chat.id]["state"] == "awaiting_deletion" and user_states[message.chat.id]["user_id"] == message.from_user.id)
